...
fn test_classical_snowflake_generator_builder_builds_configuration() {
    ...
    let id = generator.generate().expect("default immediate policy should allocate");
...
fn test_classical_snowflake_generator_builder_enforces_expiration() {
    let epoch = UNIX_EPOCH + Duration::from_secs(1_700_000_000);
    let layout = ClassicalSnowflakeLayout::new(17).expect("classic Snowflake layout must be valid");
