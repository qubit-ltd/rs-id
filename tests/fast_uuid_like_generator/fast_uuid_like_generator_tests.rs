 ...
 *
 ******************************************************************************/
//! Tests for fast UUID-like generation and formatting.

use qubit_id::{FastUuidLikeGenerator, IdGenerator, fast_simple_uuid_like, fast_uuid_like};

#[test]
fn test_fast_uuid_like_generator_formats_u128_as_canonical_uuid_like_text() {
    let value = 0x12345678_9abc_def0_1234_56789abcdef0_u128;

    assert_eq!(
        FastUuidLikeGenerator::format_uuid_like(value),
        "12345678-9abc-def0-1234-56789abcdef0"
    );
    assert_eq!(
        FastUuidLikeGenerator::format_simple_uuid_like(value),
        "123456789abcdef0123456789abcdef0"
    );
}

#[test]
fn test_fast_uuid_like_generator_next_string_uses_uuid_like_format() {
    let generator = FastUuidLikeGenerator::new();

    let id = generator.next_id().expect("uuid-like id should generate");
    let uuid = FastUuidLikeGenerator::format_uuid_like(id);
    let next = generator
        .next_string()
