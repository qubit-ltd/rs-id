...
只有新实例首次观察到的时间片不早于旧实例最后分配的时间片时，该策略才能
防止“旧实例已经停止、新实例随后启动”的无状态同片重启重复。由于它不读取或
持久化旧实例的分配水位，跨重启的时钟回拨仍可能让新实例重新进入旧实例已经
使用过的时间片并生成重复 ID。该策略也不能防止旧实例仍在运行或再次启动时的
相同身份并行冲突，并且可能保守地多等待一个完整时间片。
