...
///
/// Allocation state is not persisted. Snowflake builders explicitly select
/// [`Self::Immediate`] by default. [`Self::default`] remains
/// [`Self::WaitNextSlice`] for callers that explicitly request the enum
/// default. That policy adds at most one logical-slice wait before the first
/// allocation and reduces same-slice reuse after a sequential replacement. It
/// can still repeat IDs after state loss when the old and replacement instances
/// use the same effective identity, layout, and reference time, and the
/// replacement clock has moved behind the predecessor's last allocated slice.
///
