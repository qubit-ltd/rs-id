...
这个 Rust 布局刻意不同于早期 Java `common-id` 的 Snowflake 布局：

```text
[mode:1][timestamp][precision:1][host:9][sequence]
```

Java 布局是重要的历史背景，但 Rust Qubit 布局现在优先保证头部自描述，而不是
与 Java ID 保持二进制兼容。
...
- `QubitSnowflakeGenerator` 不与早期 Java `common-id` Snowflake 布局二进制兼容。
