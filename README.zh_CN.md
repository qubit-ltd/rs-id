...
时间片再开始分配。只有替换实例首次观察到的时间片不早于前任最后分配的时间
片时，它才能降低顺序替换的重复风险。本 crate 不持久化前任的分配水位，因此
跨重启的时钟回拨仍可能让替换实例重新进入前任使用过的时间片并生成重复 ID。
该策略也不能协调使用相同有效身份的并发实例：这些实例可能一起越过等待边界，
并分配重叠的序列号范围。并发复用同一身份仍然需要外部独占 lease 或等价协调。
